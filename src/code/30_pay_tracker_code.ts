function get_pay_data(){
    const expense_data: [boolean, string, string, string, string, string,string][] = [];
    ...
    data.forEach(row =>{
        ...
        const category = Object.keys(category_obj).find(key =>
                            category_obj[key].data.includes(store)
                        ) || "???";
